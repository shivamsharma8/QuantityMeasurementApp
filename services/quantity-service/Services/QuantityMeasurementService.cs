...
namespace QuantityMeasurementBusinessLayer
{
    public class QuantityMeasurementService : IQuantityMeasurementService
    {
        ...
        public QuantityMeasurementDto ProcessMultiply(QuantityInputDto request) => throw new NotImplementedException("Multiply is not supported.");
        ...
        private QuantityMeasurementDto ProcessOperation(QuantityInputDto request, OperationType operation)
        {
            ...
            // Note: History saving moved to HistoryService. 
            // QuantityService can potentially emit an event or return the entity if needed, 
            // but for now we provide the response back to the frontend which will then call history-service or vice versa.
            // Requirement says Quantity Service should remain public and independent.
            ...
            Func<string, U> parseUnit) where U : struct, Enum
        {
            ...
            if (operation == OperationType.Convert)
            {
                ...
                response.Result = $"{converted.Value} {converted.Unit}";
                return;
            }
            ...
            else if (operation == OperationType.Add)
            {
                ...
                response.Result = $"{sum.Value} {sum.Unit}";
            ...
            else if (operation == OperationType.Subtract)
            {
                ...
                response.Result = $"{sub.Value} {sub.Unit}";
        ...
        private WeightUnit ParseWeightUnit(string unit)
        {
            switch (unit.Trim().ToLower())
            {
                ...
                case "tonne": case "tonnes": case "t": return WeightUnit.Pound; 
